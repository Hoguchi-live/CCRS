...
\begin{frame}{Points de $l$-torsion}
	...
	\begin{align*}
		Q &= (e_0, e_1)\in \Z_{l}\times G, \text{ G abelien et }\#G = C, \\
		C\cdot Q &= (\kappa e_0, 0) \text{ pour } \kappa\in \FF_l^{*}.
	\end{align*}
...
child {node {$2P, 3P$} [sibling distance = 2cm]
	...
	child {node {$5P,6P$}
		edge from parent node [right, gray] {1}
	}
	edge from parent node [left, gray] {0}} 
child {node {$3P, 4P$} [sibling distance = 2cm]
	...
	edge from parent node [right, red] {1}};
	\end{tikzpicture}
	\end{center}
...
\begin{itemize}
	...
	\item[Prop.]{$\phi:E_A\rightarrow E_{A'}$ isogénie de noyau $\langle P \rangle$.\\
		...
		\[d = \left(\frac{A-2}{A+2}\right)^l \left( \frac{h_S(1)}{h_S(-1)}\right)^8 \text{ , et}\]
			\[\boxed{h_S(X) = \prod_{s\in S}(X-\x([s]P))}\text{, avec }S=\{1,3, \ldots, l-2\}\]}
\end{itemize}
\end{frame}
...
\begin{frame}{Isog\'enies radicales}
	\begin{itemize}
		\item Pour $l\in\left\{3, 5, 7\right\}$ .
		\item Model: $		F = y^2 + (1-c)xy - by - x^2(x-b)$.
	\end{itemize}
	On cherche des chaines de $l$-isog\'enies (ici $k$-pas)
\[
...
\begin{frame}
	...
	Mais
	\begin{itemize}
		\item Deux premiers $l$ inutilisables ($\sim 10$ bits).
		\item Structures trop rigides.
	\end{itemize}
	Optimisations potentielles:
	\begin{itemize}
		\item Parallélisation GPU/multi-threading pour la multi-\'evaluation.
		\item Plus de caching.
		\item Meilleures formules radicales.
		\item Nouvelle courbe de base.
	\end{itemize}
\end{frame}
